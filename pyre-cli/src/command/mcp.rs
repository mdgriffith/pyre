...
fn tools() -> Vec<JsonValue> {
    vec![
        ...
        tool("pyre_docs", "Return bundled Pyre documentation by topic. Start with sync for client/server integration; elm-sync and multi-database-upgrade extend it. server-contexts covers optional server-side session caching.", json!({"type":"object","properties":{"topic":{"type":"string","enum":["getting-started","schema","query","namespacing","sync","elm-sync","multi-database-upgrade","server-contexts","migrations","project-structure","serve","troubleshooting","mcp"]}}})),
