...
fn tools() -> Vec<JsonValue> {
    vec![
        ...
        tool("pyre_docs", "Return bundled Pyre documentation by topic. Read schema and query for identity and operation contracts, sync for durable TypeScript submission/recovery, ephemeral-state for transient Connection/Shared state, elm-sync for Elm builders/receipts, and seeding or rust-server for server-owned execution. multi-database-upgrade and migrations cover upgrades; server-contexts is optional session caching.", json!({"type":"object","properties":{"topic":{"type":"string","enum":DOC_RESOURCES.iter().map(|doc| doc.topic).collect::<Vec<_>>()}}})),
